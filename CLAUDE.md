...
only. It builds a universal, *unsigned* dmg+zip; the mac icon is generated by electron-builder from
`build/icon.png` (512×512, exactly the minimum), so there is no committed `.icns`. A workflow_dispatch
